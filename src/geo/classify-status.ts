...
function isLatviaZone(zone: MatchedZone): boolean {
  const c = (zone.country ?? "").toUpperCase();
  return c === "LV" || c === "LVA" || zone.source === "lgs";
}

function isLithuaniaZone(zone: MatchedZone): boolean {
  const c = (zone.country ?? "").toUpperCase();
  return c === "LT" || c === "LTU" || zone.source === "anslt";
}

function isEstoniaZone(zone: MatchedZone): boolean {
  const c = (zone.country ?? "").toUpperCase();
  return c === "EE" || c === "EST" || zone.source === "eans";
}

function isSlovakiaZone(zone: MatchedZone): boolean {
  const c = (zone.country ?? "").toUpperCase();
  return c === "SK" || c === "SVK" || zone.source === "nsat";
}

function isSloveniaZone(zone: MatchedZone): boolean {
  const c = (zone.country ?? "").toUpperCase();
