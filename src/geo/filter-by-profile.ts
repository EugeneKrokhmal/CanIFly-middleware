...
import {
  isFreeBandZone,
  isNationalPopulationAdvisory,
} from "./classify-status.js";

function reasonsIncludeMilitary(reasons: string[]): boolean {
  return reasons.some((r) => String(r).toUpperCase().includes("MILITARY"));
}

function isLowRiskResidential(zone: MatchedZone): boolean {
...
function passesWeightAndMilitary(
  zone: MatchedZone,
  profile: DroneProfile,
  ceiling: number,
): boolean {
  if (
    profile.operationCategory === "open" &&
...
export function filterByProfile(
  zones: MatchedZone[],
  profile: DroneProfile,
  altitudeAgl: number = profile.maxAltitudeAgl,
): MatchedZone[] {
  const ceiling =
    profile.operationCategory === "open"
...
export function filterForMap(
  ...
  return zones.filter((zone) => {
    if (isNationalPopulationAdvisory(zone)) return false;
    if (!altitudeOverlaps(zone, altitudeAgl)) return false;
    return passesWeightAndMilitary(zone, profile, ceiling);
  });
}
