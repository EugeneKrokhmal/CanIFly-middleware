...
export function isDipulSource(source: ZoneSource): boolean {
  return source === "dipul";
}

function pickDipulName(attrs: Record<string, unknown>): string {
  const raw =
...
export function normalizeDipulAttributes(
  ...
  const lowerUnit = String(attrs.lower_limit_unit ?? "m") as UomDimensions;
  const upperUnit = String(attrs.upper_limit_unit ?? lowerUnit) as UomDimensions;
  const lowerRaw = Number(attrs.lower_limit_altitude ?? 0);
  const upperRaw =
    attrs.upper_limit_altitude != null
  ...
  return {
    ...
    upperRef: verticalRef(
      attrs.upper_limit_alt_ref ?? attrs.lower_limit_alt_ref,
    ),
    message: legalRef,
  };
}

function buildDipulPublisher(attrs: Record<string, unknown>): ZonePublisherMeta | undefined {
  const typeCode = pickString(attrs.type_code);
  ...
  if (externalRef) extras.externalReference = externalRef;
  if (nameEn) extras.nameEn = nameEn;
  if (nameDe) extras.nameDe = nameDe;
...
function buildDipulApplicability(
  attrs: Record<string, unknown>,
 ...
 */
export function enrichDipulAttributes(
  attrs: Record<string, unknown>,
