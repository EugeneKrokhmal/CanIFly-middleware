...
const ENAIRE_SOURCES = new Set<ZoneSource>(["aero", "urbano", "infra", "servais"]);

export function isEnaireSource(source: ZoneSource): boolean {
  return ENAIRE_SOURCES.has(source);
}

/** Core MatchedZone fields from ENAIRE servAIS attribute bags. */
export function normalizeEnaireAttributes(
  ...
  const uom = String(attrs.uom ?? attrs.uomDimensions ?? "M") as UomDimensions;
...
function buildEnaireContacts(attrs: Record<string, unknown>): ZoneContact[] {
  const emails = splitDelimitedList(attrs.email);
  const phones = splitDelimitedList(attrs.phone);
...
function buildEnaireApplicability(attrs: Record<string, unknown>): ZoneApplicability | undefined {
  const dayRaw = pickString(attrs.day);
  const schedule: ZoneTimeWindow[] = [];
...
function buildEnairePublisher(attrs: Record<string, unknown>): ZonePublisherMeta | undefined {
  ...
  if (issued) extras.issued = issued;
  if (provider) extras.provider = provider;
  if (originator) extras.originator = originator;
  if (gfid) extras.gfid = gfid;
...
function buildEnaireAltitudeNotes(attrs: Record<string, unknown>): string[] | undefined {
  const notes: string[] = [];
  ...
  if (technical) notes.push(technical);
  if (restrictionConditions) notes.push(restrictionConditions);
  if (otherReason) notes.push(otherReason);
  return notes.length > 0 ? notes : undefined;
}

/** Rich ENAIRE servAIS fields mapped to the common enrichment shape. */
export function enrichEnaireAttributes(
  attrs: Record<string, unknown>,
