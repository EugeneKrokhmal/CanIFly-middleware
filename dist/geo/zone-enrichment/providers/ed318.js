...
function scheduleEntryToWindow(entry) {
    return {
...
function applicabilityToCommon(app) {
    const permanent = app.permanent != null &&
        ["true", "yes", "1", "permanent"].includes(String(app.permanent).toLowerCase());
    const schedule = app.schedule?.map(scheduleEntryToWindow).filter((w) => w.days?.length || w.startTime || w.endTime);
    const result = {
...
export function enrichEd318Feature(feature) {
    const contacts = (feature.zoneAuthority ?? [])
        .map((za) => ({
    ...
    const applicability = appBlocks.length === 1
        ? appBlocks[0]
        : appBlocks.length > 1
            ? {
