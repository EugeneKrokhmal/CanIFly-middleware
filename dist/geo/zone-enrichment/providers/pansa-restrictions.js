...
export function mapPansaRestriction(attrs) {
    ...
    if (restrictionRaw) {
        ...
        if (r.includes("PROHIB"))
            return "PROHIBITED";
        if (r.includes("AUTHORI") || r.includes("AUTH"))
...
export function pansaReasonsFromAttrs(attrs) {
    const t = String(attrs.type ?? "").toUpperCase();
    if (t.startsWith("DRA"))
        return ["UAS_GEOGRAPHIC_ZONE"];
    if ([
        "CTR",
        "CTR1KM",
        "CTR6KM",
    ...
    if (["TSA", "TRA", "D", "R", "P", "MRT"].includes(t))
        return ["OTHER"];
    return ["OTHER"];
}
function formatPansaActivity(acts) {
    if (!acts || typeof acts !== "object")
        return undefined;
    const a = acts;
    if (a.H24 === true || a.h24 === true)
        return "Zone active H24";
    if (typeof a.text === "string" && a.text.trim())
        return a.text.trim();
    return undefined;
}
export function pickPansaDescription(attrs) {
    return pickDescription(attrs.description);
}
export function formatPansaActivityFromAttrs(attrs) {
